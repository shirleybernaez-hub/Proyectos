...
export default function App() {
  ...
  // --- VISTA TV: ANCLAJE SUPERIOR ANTI-CORTE ---
  if (isTV) {
    return (
      <div className="h-screen w-screen bg-[#050505] text-white flex justify-center items-start overflow-hidden font-sans pt-16"> 
        {/* Usamos items-start y pt-16 para que todo cuelgue desde arriba y no toque el fondo */}
        
        {!enPartida ? (
          <div className="flex w-[90%] h-[80%] gap-10"> {/* Altura limitada al 80% */}
            
            <div className="w-[60%] flex flex-col h-full bg-[#24292E] rounded-[40px] overflow-hidden shadow-2xl border border-white/5">
              ...
              <div className="flex-1 flex items-center justify-center p-12">
                <img src={LogoBilliard} alt="Logo" className="max-h-[150px] w-auto object-contain" />
              </div>
              
              {/* Bloque Blanco elevado */}
              <div className="bg-white text-black flex items-center p-10 gap-8 min-h-[220px]">
                <div className="w-40 h-40 bg-white rounded-2xl flex items-center justify-center p-4 shadow-xl border border-black/5">
                  <QRCodeSVG value={qrUrl} size="100%" />
                </div>
                <div className="flex flex-col justify-center">
                  <p className="text-[14px] font-bold tracking-[0.3em] uppercase mb-1 text-black">Mesa {mesaId.replace("mesa", "")} Disponible</p>
            ...
            <div className="w-[40%] h-full flex flex-col">
               <div className="flex-1 rounded-[40px] bg-[#1A1A1A] border border-white/5 flex flex-col items-center justify-end p-16 relative">
                  <span className="absolute top-10 text-[10px] tracking-[0.5em] text-white/20 uppercase font-bold">Espacio Disponible</span>
                  <p className="text-gray-500 text-5xl font-normal tracking-[0.1em] uppercase mb-10">PUBLICIDAD</p>
               </div>
            </div>
          </div>
        ) : (
          /* Marcador en vivo centrado verticalmente */
