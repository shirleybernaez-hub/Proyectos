...
function Login({ onLoginSuccess }) {
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-6">
      <form onSubmit={handleLogin} className="w-full max-w-md bg-[#111] p-10 rounded-[2.5rem] border border-white/5 flex flex-col gap-4">
        <h2 className="text-xl font-black text-center mb-4 uppercase">Login Panel</h2>
        ...
        <button className="bg-white text-black font-black p-5 rounded-2xl uppercase mt-4 active:scale-95">Ingresar</button>
      </form>
    </div>
  );
}
...
export default function App() {
  ...
  // Si se pide explícitamente el dashboard, mostramos login
  if (isDash && !clienteSession) {
    return <Login onLoginSuccess={setClienteSession} />;
  }
  
  if (isDash && clienteSession) {
    return <div className="p-20 text-white font-black uppercase text-center">Panel de {clienteSession.nombreLocal} próximamente</div>;
  }
...
function ShotClockMobile({ data, mesaId }) {
  ...
  return (
    <div className="w-full flex flex-col gap-2">
      <button onClick={() => updateDoc(doc(db,"mesas",mesaId),{shotActive:false, tiempoShot:maxTime})} className="flex items-center text-[10px] font-black text-white/40 uppercase ml-1 active:text-white/70"><IconReset /> REINICIAR TIEMPO</button>
